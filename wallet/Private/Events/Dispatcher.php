...
class Dispatcher
{
    ...
    public static function callEventWaiting(string $classAlaise, string $eventMethod) : void 
    {
        ...
        if (method_exists($eventClass, $eventMethod)) :
            ...
            // call class method
            call_user_func_array([$eventClass, $eventMethod], $argumentsNew);
        ...
        if (isset($eventWaiting[$classAlaise . '.' . $eventMethod])) :
            ...
            if (isset($event[0]) && is_callable($event[0])) :
                ...
                // call closure
                call_user_func_array($closure->bindTo($eventClass, \get_class($eventClass)), $arguments);
        ...
        if (count($closureWaiting) > 0) :
            ...
            foreach ($closureWaiting as $closure) call_user_func_array($closure, $closureArgument);
        ...
        if (isset($attachedEvent['callback'])) :
            ...
            // call closure
            call_user_func_array($attachedEvent['callback']->bindTo($eventClass, \get_class($eventClass)), $arguments);
