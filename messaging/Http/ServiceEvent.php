...
class ServiceEvent
{
    ...
    private static $eventNames =[
        ...
        'new sender mail' => [
            'method' => 'POST',
            'endpoint' => 'mail/sendMailToSender',
            'query' => []
        ],

        'new account login' => [
            'method' => 'POST',
            'endpoint' => 'mail/accountLoginAlert',
            'query' => []
        ],

        'new password reset' => [
            'method' => 'POST',
            'endpoint' => 'mail/sendPasswordResetMail',
            'query' => []
        ],

        'password reset complete' => [
            'method' => 'POST',
            'endpoint' => 'mail/sendCompletePasswordResetMail',
            'query' => []
        ],

        'new downline added' => [
            'method' => 'POST',
            'endpoint' => 'mail/sendNewDownlineAlert',
            'query' => []
        ],

        'email approved' => [
            'method' => 'POST',
            'endpoint' => 'mail/sendEmailConfirmation',
            'query' => []
        ],

        'send otp' => [
            'method' => 'POST',
            'endpoint' => 'mail/sendOTP',
            'query' => []
        ],

        'pickup processed' => [
            'method' => 'POST',
            'endpoint' => 'sms/pickupProcessed',
            'query' => []
        ],

        'new pickup request' => [
            'method' => 'POST',
            'endpoint' => 'sms/newPickup',
            'query' => []
        ],

        'rider gone offline' => [
            'method' => 'POST',
            'endpoint' => 'sms/riderGoneOffline',
            'query' => []
        ],

        'pickup approved' => [
            'method' => 'POST',
            'endpoint' => 'sms/pickupApproved',
            'query' => []
        ]
    ];
                ...
                if ($jsonObject !== null) :
                    ...
                    foreach ($jsonObject as $serviceName => $serviceUrl) :
                        ...
                        if ($serviceName == $service) :
                            ...
                            HttpRequest::header([
                                'x-service-event' => $event,
                                'x-service-data' => json_encode($data),
                                'x-micro-services' => $headers['x-micro-services']
                            ]);
